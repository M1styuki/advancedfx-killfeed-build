...
constexpr unsigned char kClientHash[] = {
    0xa0,0xc1,0x95,0xf0,0xb6,0xec,0x00,0x91,0x5e,0xf0,0x8c,0x54,0x82,0x00,0xa0,0x10,
    0xeb,0xbe,0x79,0x82,0xd3,0xa4,0xbc,0x46,0x8c,0xad,0x93,0x9b,0x67,0xc8,0xc4,0xe3
};
...
void * ResolveUi(uint64_t handle) {
    ...
    auto engine = *reinterpret_cast<void **>(g_Base + 0x258F010);
...
void ApplyBuyStyles(void * menu) {
    ...
    const auto cantBuy = *reinterpret_cast<uint16_t *>(g_Base + 0x242F450);
    const auto cantAfford = *reinterpret_cast<uint16_t *>(g_Base + 0x242F44C);
...
void * ResolveMenu() {
    ...
    void * engine = *reinterpret_cast<void **>(g_Base + 0x258F010);
...
void * ObservedLoadout(void * inventory, unsigned int team, unsigned int slot) {
    if(!g_Controller || inventory != Field<void *>(g_Controller, 0x818)) return nullptr;
    ...
    auto manager = reinterpret_cast<void * (__fastcall *)()>(g_Base + 0x813A90)();
        ...
        for(int i = 0; i < count; ++i) {
            ...
            if(Field<uint16_t>(record, 48) == team && Field<uint16_t>(record, 50) == slot) {
                auto item = reinterpret_cast<void * (__fastcall *)(void *, unsigned int, int, int)>(
                    g_Base + 0x10B5BB0)(manager, Field<uint16_t>(record, 52), 0, 0);
                if(item) { ++g_LoadoutHits; return item; }
            }
        }
    }
    // The server vector is sparse: absent entries use the shared default
    // definition table, exactly as 8DCA60 does. 816640 is a direct team/slot
    // array lookup; it does not read the local user's equipped-item interface.
    {
        auto item = reinterpret_cast<LoadoutFn>(g_Base + 0x816640)(manager, team, slot);
        if(item) { ++g_LoadoutHits; return item; }
    }
    if(slot <= 56) ++g_LoadoutMisses; // Ignore native hover/special-slot sentinels.
    return nullptr;
}
...
intptr_t __fastcall SetModel(void * preview, const char * model) {
    if(g_Depth && g_Pawn && preview == g_ModelPreview) {
        ...
        model = nullptr;
        reinterpret_cast<void * (__fastcall *)(void *, const char **)>(g_Base + 0x21D2B0)(g_Pawn, &model);
        if(!model || !*model) model = g_Pawn->GetTeam() == 3
            ? "agents/models/ctm_sas/ctm_sas.vmdl" : "agents/models/tm_phoenix/tm_phoenix.vmdl";
        // DF3210 stores the viewer's agent item ID in this native preview slot.
        // Clear that cosmetic binding before applying the recorded model.
        const int slot = Field<int>(preview, 2252);
        auto records = Field<unsigned char *>(preview, 2264);
        if(records && slot >= 0 && slot < Field<int>(preview, 2256))
            Field<uint64_t>(records + 152 * slot, 40) = 0;
    }
...
void MirvPovBuyMenu_Initialize(HMODULE module) {
    if(g_Hooked) return;
    if(!module || !MatchesBuild(module) || g_clientDllOffsets.C_CSPlayerPawn.m_bIsBuyMenuOpen != 0x150A) {
    ...
    reinterpret_cast<void (__fastcall *)(uint16_t *, const char *)>(g_Base + 0x166B6E0)(
        &g_HudWashClass, "hud-colorize-wash");
    g_Open = reinterpret_cast<PanelFn>(g_Base + 0xD55A00);
    g_Close = reinterpret_cast<PanelFn>(g_Base + 0xD416D0);
    g_Refresh = reinterpret_cast<PanelFn>(g_Base + 0xD598A0);
    g_FullRefresh = reinterpret_cast<PanelFn>(g_Base + 0xD59970);
    g_Hover = reinterpret_cast<PanelFn>(g_Base + 0xD614C0);
    g_Think = reinterpret_cast<PanelFn>(g_Base + 0xD47C70);
    g_GetPawn = reinterpret_cast<GetterFn>(g_Base + 0x93F7C0);
    g_GetController = reinterpret_cast<GetterFn>(g_Base + 0x93F780);
    g_Loadout = reinterpret_cast<LoadoutFn>(g_Base + 0x8DBEB0);
    g_LoadoutHover = reinterpret_cast<LoadoutFn>(g_Base + 0x8DBE10);
    g_Write = reinterpret_cast<WriteFn>(g_Base + 0xC40010);
    g_Purchase = reinterpret_cast<PurchaseFn>(g_Base + 0xD48990);
    g_Sell = reinterpret_cast<SellFn>(g_Base + 0xD48CD0);
    g_Model = reinterpret_cast<ModelFn>(g_Base + 0xD5F9D0);
    g_Select = reinterpret_cast<SelectFn>(g_Base + 0xD550F0);
    g_SetModel = reinterpret_cast<SetModelFn>(g_Base + 0xDF33F0);
...
void MirvPovBuyMenu_Update() {
    ...
    if(g_Panel && tickChanged) {
        ...
        if(!Dispatch(0xD50560)) g_Pending = false;
    }
}
...
void MirvPovBuyMenu_PrintStatus() {
    ...
    for(int i = 1; i <= GetHighestEntityIndex(); ++i) {
        ...
        auto inventory = Field<void *>(controller, 0x818);
        advancedfx::Message("[mirv_pov_buymenu] player=%d name=%s open=%d team=%d loadout_count=%d\n",
            i, controller->GetPlayerName(), Field<uint8_t>(pawn, 0x150A) != 0, pawn->GetTeam(),
    ...
    if(g_Panel && g_Pawn && g_Controller) {
        ...
        if(preview) {
            auto map = Field<void *>(preview, 104);
            auto records = Field<unsigned char *>(preview, 2264);
            const int count = Field<int>(preview, 2256);
            advancedfx::Message("[mirv_pov_buymenu] preview=%p visible=%d item=%llu slots=%d pending=%d render=%d map_loaded=%d map_entities=%d\n",
                preview, Visible(preview), Field<uint64_t>(g_Panel, 528), count,
                Field<uint8_t>(preview, 88), Field<uint8_t>(preview, 1713),
                map ? Field<int>(map, 152) : -1, map ? Field<int>(map, 208) : -1);
            if(records && count > 0) advancedfx::Message("[mirv_pov_buymenu] preview_model=%s entity=%u dirty=%d\n",
                Field<const char *>(records, 24) ? Field<const char *>(records, 24) : "", Field<uint32_t>(records, 0), Field<uint8_t>(preview, 2208));
        ...
        for(int group = 0; group < 5; ++group) {
            ...
            for(int i = 0; i < count; ++i) {
                ...
                const int result = reinterpret_cast<int (__fastcall *)(void *, void *, int, int *)>(
                    g_Base + 0x8988C0)(Field<void *>(g_Pawn, 0x1210), item, 1, nullptr);
                auto owned = reinterpret_cast<void * (__fastcall *)(void *, void *)>(g_Base + 0x8D6F90)(
                    Field<void *>(g_Pawn, 0x1208), item);
